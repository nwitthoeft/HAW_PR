...
void main()
{
	...
	do
	{
		...
		printf("Was ist die Obergrenze? (Um das Projekt zu beenden geben sie b ein)\n");
		scanf_s("%d", &eingabe);
		if (eingabe == 'b')
		{
			break;
		...
		{
			for (k = 2; k <= eingabe; k++) //zählt bis zur Eingabe hoch.
			{
				for (i = 2; i <= k; i++) //zählt bis zur hochgezählten variable hoch.
				{
